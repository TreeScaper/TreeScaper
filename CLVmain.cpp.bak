...
TreeSetObjects<Container_type> *trees_driver(map<String, String> &paras, Container_type dummy)
{
	...
	String outname_Taxa("Taxon");
	outname_Taxa.make_stdname(paras);
	Header_info Header_Taxa;
	Header_Taxa.insert("created", time_stamp());
	Header_Taxa.insert("output type", "Taxon list");
	Header_Taxa.insert("size", to_string(Taxa_ptr->size));
	Header_Taxa.insert("source", paras["-f"]);
	fout.open((char *)outname_Taxa);
	fout << Header_Taxa;
	for (auto i = 0; i < Taxa_ptr->size; i++)
		fout << i << '\t' << Taxa_ptr->Ind2Taxon(i) << '\n';
	fout.close();
	cout << "Sucessfully printed taxon list info in Taxon_" << paras["-post"] << ".out file.\n\n";
	...
	else if (paras["-output"] == (String) "DIS")
	{
		...
		std::cout << "Computing RF-distance matrix, new approach...\n";
		treesobj_ptr->release_dis_mat();
		start = std::clock();
		if (flag_subtree)
			treesobj_ptr->Compute_RF_Distance_Matrix(subset, id_mapping);
		else
			treesobj_ptr->Compute_RF_Distance_Matrix_new();
		end = std::clock();
		cout << "Compute RF-distance matrix time(s), new approach:\t" << (end - start) / (double)CLOCKS_PER_SEC << "\n";
...
bool reload_driver(map<String, String> &paras, Container_type dummy)
{
	...
	if (filetype)
	{
		...
		else if (paras["-output"] == (String) "DIS")
		{
			...
			std::cout << "Computing RF-distance matrix faster...\n";
			start = std::clock();
			...
			if (tree_s)
				...
				treesobj.Compute_RF_Distance_Matrix_new();
...
bool comm_driver(SpecMat::LowerTri<PRECISION>* Adj, map<String, String> &paras)
{
	...
	cout << "Read graph time(s):\t\t" << (end - start) / (double)CLOCKS_PER_SEC << "\n";
